...
func convertToVerkle(ctx *cli.Context) error {
	...
	snaptree, err := snapshot.New(snapshot.Config{CacheSize: 256, NoBuild: true}, chaindb, trie.NewDatabase(chaindb), root)
	if err != nil {
		return err
	}
...
func convertSnapshotAcc(chaindb ethdb.Database, vRoot *verkle.InternalNode, epoch verkle.StateEpoch, enableStateExpiry bool, nodeResolver func(path []byte) ([]byte, error), saveverkle func(path []byte, node verkle.VerkleNode)) error {
	...
	for it.Next() {
		...
		acc, err := types.FullAccount(it.Value())
		if err != nil {
			log.Error("Invalid account encountered during traversal", "error", err)
			return err
		}
	...
	if it.Error() != nil {
		log.Error("Failed to compute commitment", "error", it.Error())
		return it.Error()
	}
	log.Info("Wrote all accounts", "count", count, "elapsed", common.PrettyDuration(time.Since(start)))
