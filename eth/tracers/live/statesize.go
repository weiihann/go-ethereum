...
// CSV column headers (delta values per block)
...
// stateSizeRecord represents a single CSV record with delta stats.
...
func (s *stateSizeTracer) onStateUpdate(update *tracing.StateUpdate) {
	...
	// Calculate state size delta and depth counts
...
// It returns the delta and depth counts for account/storage trie nodes (created and deleted).
func calculateStateSizeDelta(update *tracing.StateUpdate) (
	delta stateSizeDelta,
	accountDepthCreated, storageDepthCreated, accountDepthDeleted, storageDepthDeleted [65]int64,
) {
...
func (s *stateSizeTracer) writeRecord(r stateSizeRecord) {
	row := []string{
		...
		r.Root.Hex(),
		strconv.FormatInt(r.Delta.AccountDelta, 10),
		strconv.FormatInt(r.Delta.AccountBytesDelta, 10),
		strconv.FormatInt(r.Delta.AccountTrienodeDelta, 10),
		strconv.FormatInt(r.Delta.AccountTrienodeBytesDelta, 10),
		strconv.FormatInt(r.Delta.ContractCodeDelta, 10),
		strconv.FormatInt(r.Delta.ContractCodeBytesDelta, 10),
		strconv.FormatInt(r.Delta.StorageDelta, 10),
		strconv.FormatInt(r.Delta.StorageBytesDelta, 10),
		strconv.FormatInt(r.Delta.StorageTrienodeDelta, 10),
		strconv.FormatInt(r.Delta.StorageTrienodeBytesDelta, 10),
	}
