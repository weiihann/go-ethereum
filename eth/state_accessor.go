...
func (eth *Ethereum) StateAtBlock(ctx context.Context, block *types.Block, reexec uint64, base *state.StateDB, readOnly bool, preferDisk bool) (statedb *state.StateDB, release tracers.StateReleaseFunc, err error) {
	var (
		...
		curPeriod = types.GetStatePeriod(eth.blockchain.Config(), block.Time())
	)
	...
	if readOnly {
		...
		if statedb, err = eth.blockchain.StateAt(block.Root(), curPeriod); err == nil {
	...
	if base != nil {
		if preferDisk {
			...
			if statedb, err = state.New(block.Root(), database, nil, curPeriod); err == nil {
		...
		if !readOnly {
			statedb, err = state.New(current.Root(), database, nil, curPeriod)
			if err == nil {
				return statedb, noopReleaser, nil
			}
		}
	...
	for current.NumberU64() < origin {
		...
		statedb, err = state.New(root, database, nil, curPeriod)
		if err != nil {
