...
// offset and size.
//
// The fsync is the caller's responsibility (Sync); WriteSubtree only
// appends to the OS page cache so that one fsync can be amortised over
// many subtrees (critical at hundreds of millions of subtrees).
...
// RecordsEncodedSize returns the number of bytes WriteSubtree would append for
// the given records, without writing anything. Used to size dry runs.
func RecordsEncodedSize(records []*Record) (uint64, error) {
	var size uint64
	for _, rec := range records {
		encoded, err := rlp.EncodeToBytes(rec)
		if err != nil {
