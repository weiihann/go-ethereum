...
// Package archive implements the EIP-8188 "expired node" archive format
// (gballet/archival-command): moved-out trie subtrees are stored in a flat
// append-only file as a sequence of RLP Record{Path, Value} entries holding
// only the subtree's leaves; a 17-byte expired-node pointer in chaindb brackets
// each subtree's records by (offset, size). This package is the read side and
// the file writer; the trie package supplies the leaf encoder and the
