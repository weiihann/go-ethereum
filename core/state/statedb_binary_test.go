...
// TestBinaryTrieParallelZoneUpdates verifies that the parallel zone update
// path (SplitRoot → concurrent left/right → MergeRoot) produces correct
// results. It first populates the trie (so the root becomes an InternalNode),
// then applies a second round of mutations that exercises the parallel path.
func TestBinaryTrieParallelZoneUpdates(t *testing.T) {
	state := newBinaryStateDB(t)

	addr1 := common.HexToAddress("0x1111111111111111111111111111111111111111")
	addr2 := common.HexToAddress("0x2222222222222222222222222222222222222222")

	// Round 1: populate the trie so the root becomes an InternalNode.
	// This round uses the sequential fallback (empty trie → SplitRoot fails).
	...
	state.SetBalance(addr2, uint256.NewInt(1e18), tracing.BalanceChangeUnspecified)
	state.SetCode(addr2, []byte{0x60, 0x00, 0x60, 0x00, 0xf3}, tracing.CodeChangeUnspecified)
	state.SetState(addr2, common.Hash{31: 0}, common.HexToHash("0x01"))   // zone 000
	state.SetState(addr2, common.Hash{31: 100}, common.HexToHash("0x01")) // zone 1
	state.IntermediateRoot(false)

	// Round 2: mutate state again — this time the root is an InternalNode,
	// so SplitRoot succeeds and the parallel path is exercised.
	state.SetNonce(addr1, 5, tracing.NonceChangeUnspecified)
	state.SetBalance(addr2, uint256.NewInt(2e18), tracing.BalanceChangeUnspecified)

	// Header storage (zone 000): slots 0, 10, 63.
	state.SetState(addr2, common.Hash{31: 0}, common.HexToHash("0xaa"))
	state.SetState(addr2, common.Hash{31: 10}, common.HexToHash("0xbb"))
	state.SetState(addr2, common.Hash{31: 63}, common.HexToHash("0xcc"))

	// Main storage (zone 1): slots 64, 100, 256, 1000.
	state.SetState(addr2, common.Hash{31: 64}, common.HexToHash("0xdd"))
	state.SetState(addr2, common.Hash{31: 100}, common.HexToHash("0xee"))
	state.SetState(addr2, common.Hash{30: 1, 31: 0}, common.HexToHash("0xff"))   // slot 256
	state.SetState(addr2, common.Hash{30: 3, 31: 232}, common.HexToHash("0x11")) // slot 1000

	root := state.IntermediateRoot(false)
	if root == (common.Hash{}) {
		t.Fatal("state root is zero")
	}

	// Verify all values are readable from the trie after parallel updates.
	if nonce := state.GetNonce(addr1); nonce != 5 {
		t.Errorf("addr1 nonce: got %d, want 5", nonce)
	}
	if bal := state.GetBalance(addr2); bal.Cmp(uint256.NewInt(2e18)) != 0 {
		t.Errorf("addr2 balance: got %v, want 2e18", bal)
	}

	// Header storage (zone 000).
	if v := state.GetState(addr2, common.Hash{31: 0}); v != common.HexToHash("0xaa") {
		t.Errorf("slot 0: got %x, want 0xaa", v)
	}
	if v := state.GetState(addr2, common.Hash{31: 10}); v != common.HexToHash("0xbb") {
		t.Errorf("slot 10: got %x, want 0xbb", v)
	}
	if v := state.GetState(addr2, common.Hash{31: 63}); v != common.HexToHash("0xcc") {
		t.Errorf("slot 63: got %x, want 0xcc", v)
	}

	// Main storage (zone 1).
	if v := state.GetState(addr2, common.Hash{31: 64}); v != common.HexToHash("0xdd") {
		t.Errorf("slot 64: got %x, want 0xdd", v)
	}
	if v := state.GetState(addr2, common.Hash{31: 100}); v != common.HexToHash("0xee") {
		t.Errorf("slot 100: got %x, want 0xee", v)
	}
	if v := state.GetState(addr2, common.Hash{30: 1, 31: 0}); v != common.HexToHash("0xff") {
		t.Errorf("slot 256: got %x, want 0xff", v)
	}
	if v := state.GetState(addr2, common.Hash{30: 3, 31: 232}); v != common.HexToHash("0x11") {
		t.Errorf("slot 1000: got %x, want 0x11", v)
	}
}
...
func TestBinaryTrieCommitAndReopen(t *testing.T) {
	...
	root1, err := state.Commit(0, true, false)
	if err != nil {
...
func TestBinaryTrieMultiBlockTransitions(t *testing.T) {
	...
	root2, err := state.Commit(1, true, false)
	if err != nil {
	...
	root3, err := state.Commit(2, true, false)
	if err != nil {
...
// TestBinaryTrieParallelZoneConsistency verifies that parallel and sequential
// paths produce identical state roots for the same mutations. Uses two rounds
// so the second IntermediateRoot exercises the parallel path.
func TestBinaryTrieParallelZoneConsistency(t *testing.T) {
	buildState := func() common.Hash {
		state := newBinaryStateDB(t)

		// Round 1: populate trie (sequential fallback, empty root).
		for i := range 10 {
			addr := common.BigToAddress(uint256.NewInt(uint64(0x1000 + i)).ToBig())
			state.SetBalance(addr, uint256.NewInt(1e18), tracing.BalanceChangeUnspecified)
			state.SetState(addr, common.Hash{31: 100}, common.HexToHash("0x01")) // zone 1
		}
		state.IntermediateRoot(false)

		// Round 2: mutate again — now root is InternalNode, parallel path runs.
		for i := range 10 {
			addr := common.BigToAddress(uint256.NewInt(uint64(0x1000 + i)).ToBig())
			state.SetBalance(addr, uint256.NewInt(uint64(i+1)*1e18), tracing.BalanceChangeUnspecified)
			state.SetNonce(addr, uint64(i), tracing.NonceChangeUnspecified)

			for s := range 100 {
				var slotKey common.Hash
				slotKey[31] = byte(s % 256)
				slotKey[30] = byte(s / 256)
				var val common.Hash
				val[31] = byte(s + 1)
				state.SetState(addr, slotKey, val)
			}
		}
		return state.IntermediateRoot(false)
	}
	...
	if root1 != root2 {
		t.Fatalf("state roots differ: %x vs %x", root1, root2)
	}
