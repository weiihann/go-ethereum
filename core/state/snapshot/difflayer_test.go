...
func TestMergeBasics(t *testing.T) {
	...
	parent := newDiffLayer(emptyLayer(), common.Hash{}, copyDestructs(destructs), copyAccounts(accounts), copyStorage(storage), 0)
	child := newDiffLayer(parent, common.Hash{}, copyDestructs(destructs), copyAccounts(accounts), copyStorage(storage), 0)
	child = newDiffLayer(child, common.Hash{}, copyDestructs(destructs), copyAccounts(accounts), copyStorage(storage), 0)
	child = newDiffLayer(child, common.Hash{}, copyDestructs(destructs), copyAccounts(accounts), copyStorage(storage), 0)
	child = newDiffLayer(child, common.Hash{}, copyDestructs(destructs), copyAccounts(accounts), copyStorage(storage), 0)
	// And flatten
...
func TestMergeDelete(t *testing.T) {
	...
	parent := newDiffLayer(emptyLayer(), common.Hash{}, flipDrops(), flipAccs(), storage, 0)
	child := parent.Update(common.Hash{}, flopDrops(), flopAccs(), storage, 0)
	child = child.Update(common.Hash{}, flipDrops(), flipAccs(), storage, 0)
	child = child.Update(common.Hash{}, flopDrops(), flopAccs(), storage, 0)
	child = child.Update(common.Hash{}, flipDrops(), flipAccs(), storage, 0)
	child = child.Update(common.Hash{}, flopDrops(), flopAccs(), storage, 0)
	child = child.Update(common.Hash{}, flipDrops(), flipAccs(), storage, 0)
...
func TestInsertAndMerge(t *testing.T) {
	...
	)
	{
		...
		parent = newDiffLayer(emptyLayer(), common.Hash{}, destructs, accounts, storage, 0)
	}
	{
		var (
			destructs = make(map[common.Hash]struct{})
		...
		child = newDiffLayer(parent, common.Hash{}, destructs, accounts, storage, 0)
	}
	// And flatten
	merged := (child.flatten()).(*diffLayer)
...
func BenchmarkSearch(b *testing.B) {
	...
	fill := func(parent snapshot) *diffLayer {
		...
		return newDiffLayer(parent, common.Hash{}, destructs, accounts, storage, 0)
	}
	var layer snapshot
	layer = emptyLayer()
	for i := 0; i < 128; i++ {
