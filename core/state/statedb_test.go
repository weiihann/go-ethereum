...
func TestUpdateLeaks(t *testing.T) {
	...
	state, _ := New(types.EmptyRootHash, NewDatabase(db), nil, types.Period0)
...
func TestIntermediateLeaks(t *testing.T) {
	...
	transState, _ := New(types.EmptyRootHash, NewDatabase(transDb), nil, types.Period0)
	finalState, _ := New(types.EmptyRootHash, NewDatabase(finalDb), nil, types.Period0)
...
func TestCopy(t *testing.T) {
	...
	orig, _ := New(types.EmptyRootHash, NewDatabase(rawdb.NewMemoryDatabase()), nil, types.Period0)
...
func (test *snapshotTest) run() bool {
	...
	var (
		state, _     = New(types.EmptyRootHash, NewDatabase(rawdb.NewMemoryDatabase()), nil, types.Period0)
	...
	for sindex--; sindex >= 0; sindex-- {
		checkstate, _ := New(types.EmptyRootHash, state.Database(), nil, types.Period0)
...
func TestTouchDelete(t *testing.T) {
	...
	s.state, _ = New(root, s.state.db, s.state.snaps, types.Period0)
...
func TestCopyCommitCopy(t *testing.T) {
	...
	state, _ := New(types.EmptyRootHash, tdb, nil, types.Period0)
	...
	state, _ = New(root, tdb, nil, types.Period0)
	if balance := state.GetBalance(addr); balance.Cmp(big.NewInt(42)) != 0 {
...
func TestDeleteCreateRevert(t *testing.T) {
	...
	root, _ := state.Commit(0, false)
	state, _ = New(root, state.db, state.snaps, types.Period0)
...
func TestMissingTrieNodes(t *testing.T) {
	...
	state, err := New(types.EmptyRootHash, db, nil, types.Period0)
	if err != nil {
		panic("nil stte")
	}
	addr := common.BytesToAddress([]byte("so"))
	...
	state, _ = New(root, db, nil, types.Period0)
...
func TestStateDBAccessList(t *testing.T) {
	...
	memDb := rawdb.NewMemoryDatabase()
	db := NewDatabase(memDb)
	state, _ := New(types.EmptyRootHash, db, nil, types.Period0)
...
func TestFlushOrderDataLoss(t *testing.T) {
	...
	var (
		...
		state, _ = New(types.EmptyRootHash, statedb, nil, types.Period0)
	)
	for a := byte(0); a < 10; a++ {
	...
	state, err = New(root, NewDatabase(memdb), nil, types.Period0)
	if err != nil {
...
func TestResetObject(t *testing.T) {
	var (
		...
		state, _ = New(types.EmptyRootHash, db, snaps, types.Period0)
...
func TestCodeChunkOverflow(t *testing.T) {
	// Create an empty state database
	db := rawdb.NewMemoryDatabase()
	state, _ := New(common.Hash{}, NewDatabaseWithConfig(db, nil), nil, types.Period0)
